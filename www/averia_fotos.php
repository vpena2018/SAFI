...
function render_foto_thumbnail_averia($row, $mostrar_borrar = false, $puede_borrar_por_permiso = false) {
...
$sql_fotos_averia = "SELECT averia_foto.id, averia_foto.archivo, averia_foto.fecha, averia.id_estado, 'actual' as origen
    FROM averia_foto
    LEFT OUTER JOIN averia ON (averia.id = averia_foto.id_maestro)
    WHERE averia_foto.id_maestro = $cid and averia.id_producto = $pid";

if ($tiene_insp) {
    $sql = "SELECT inspeccion_foto.id, inspeccion_foto.archivo, inspeccion_foto.fecha, 0 as id_estado, 'inspeccion' as origen
    ...
    $sql = "$sql_fotos_averia ORDER BY fecha, id";
}
...
if ($result != false && $result->num_rows > 0) {
    $puede_borrar_por_permiso = tiene_permiso(151);
    while ($row = $result->fetch_assoc()) {
        ...
        render_foto_thumbnail_averia($row, $mostrar_borrar, $puede_borrar_por_permiso);
    }
        ...
        if (json.length > 0) {
            ...
            if (json[0].pcode == 1) {
                ...
                if (puede_agregar_varias) {
                    ...
                    thumb_agregar2(archivoFinal, campo, puede_agregar_varias);
                } else {
