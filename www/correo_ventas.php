 ...
 if (app_enviar_email==true) 
{
    ...
    $correo_servicio_result = sql_select("SELECT ventas.* 
        ...
        LEFT OUTER JOIN usuario l1 ON (ventas.id_vendedor=l1.id)
        ...
        $email_enviar=trim($correo_row["correos"]);
        $subject='Venta de vehículo'; 
        $cuerpo_html="
        Notificación de venta de vehículo   
        <br><br>            
        <b>Tienda:</b> ".$correo_row["latienda"]."<br><br>";
        if (!es_nulo($correo_row['fecha_vendido'])){
            $cuerpo_html.= "<b>Fecha Vendido:</b> ".formato_fecha_de_mysql($correo_row["fecha_vendido"]);
        }else{
            $cuerpo_html.= "<b>Fecha Negociacion:</b> ".formato_fecha_de_mysql($correo_row["fecha_negocacion"]);            
        }        
        $cuerpo_html.="        
        <br><br>
        <b>Vendedor:</b> ".$correo_row["vendedor"]."
        <br><br>
        <b>Vehículo:</b> ".$correo_row["codigo_alterno"]."  ".$correo_row["nombre"]."  Placa: ".$correo_row["placa"]."
        <br><br>
        <b>Estado:</b> ".$correo_row["elestado"]."
        <br><br>
