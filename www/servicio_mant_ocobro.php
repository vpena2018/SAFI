...
function  cargar_detalle_ocobro($cid,$tipo,$sql_detalle){
    ...
    $servicios_result = sql_select("SELECT * FROM servicio_detalle where  ($sql_detalle) and id_servicio=$cid  and  estado<>4 order by producto_tipo desc,id ");
        ...
        if ($servicios_result->num_rows > 0) { 
          while ($detalle = $servicios_result -> fetch_assoc()) {
            ...
            $salida.= agregar_servicio_detalle_ocobro($lin,$detalle["precio_venta"],$detalle["precio_costo"],$detalle["id_producto"],$detalle["producto_codigoalterno"],$detalle["producto_nombre"],$detalle["id"],$detalle["cantidad"],$detalle["producto_nota"],$detalle["estado"]);
                  
            $lin++;
            if ($detalle["estado"]==1) {$actividad_noautorizada=true;}
            }
            if ($actividad_noautorizada==true) {
              $salida='<tr><td colspan="7" >Algunas actividades/repuestos NO estan Autorizados, El estado debe estar al menos autorizado para poder continuar<td> </tr>';
            }
            echo $salida;
            $totlin=$lin;

          } else {
                echo '<tr><td colspan="7" >No se encontraron registros que puedan ser modificados<td> </tr>';
            }
      
    } 
  
  
  function agregar_servicio_detalle_ocobro($vlin,$data_pv,$data_pc,$data_id,$data_alt,$data_desc,$det_id,$cantidad,$nota,$estado) {
    $salidatxt="";
    $devueltoclass="";
  ...
  //  $salidatxt.='<td> <input class="serv_chk_dlg" type="checkbox" value="'.$det_id.'" name="det_id_chk[]"></td> ';
   ...
   // $salidatxt.='<td><span name="totlinea" ></span></td> ';
...
if ($accion =="ocobro") { //crear ORDEN COBRO
   ...
   //validar solo ordenes completadas pueden crear orden de cobro

      $solo_completada= get_dato_sql('servicio',"id_estado"," where id=$cid"); 
      if ($solo_completada<22) {
          $verror="Unicamente las ordenes de servicio en estado completado pueden generar orden de cobro";
          echo "<br><br><br>".mensaje($verror,'warning')."<br><br><br>";
          exit;
      }
    ...
    if (isset($_REQUEST['g'])) { //Guardar orden cobro
        ...
        if (isset($_REQUEST['tipo_cobro'])) { $tipo_cobro = intval($_REQUEST['tipo_cobro']); } else	{$tipo_cobro ="" ;}
    
   //Validar
   $verror="";
    if (es_nulo($tipo_cobro)) { $verror="Debe seleccionar el Tipo de cobro";}
   if (es_nulo($id_entidad)) { $verror="Debe seleccionar el Cliente";}
           ...
           LIMIT 1");
            ...
            //Calculos
             ...
             $tmpsql="";$tmpsql2="";
               ...
               //actualizar
               $tmpsql="UPDATE servicio_detalle SET id_ocobro=:codigo WHERE id=".$detalleid." LIMIT 1";
   ...
   if ($verror=="") {
    ...
    $sqlcampos="";
     ...
     $sqlcampos.= " , numero =".GetSQLValue($numero_oc,"int"); 
      ...
      $sqlcampos.= " , tipo_cobro =".GetSQLValue($tipo_cobro,"int");
...
<div class="col">
  ...
  <div class="col-md-6">
  <?php 
     $cliente_result =sql_select("SELECT entidad.id,entidad.nombre FROM servicio LEFT OUTER JOIN entidad ON (servicio.cliente_id=entidad.id) WHERE servicio.id=$cid LIMIT 1");
...
<div class="row">
    <div class="col">
        <?php 
          echo campo("tipo_cobro","Tipo de Cobro",'select',valores_combobox_texto('
            <option value="1">Cobrable al costo sin recargo del 15% en actividades y repuestos sin gastos administrativos y sin 15% ISV</option>
            <option value="2">Cobrable al costo sin recargo en actividades y repuestos con gastos administrativos y sin 15% ISV</option>
            <option value="3">Cobrable al costo sin recargo en actividades y repuestos con gastos administrativos y con 15% ISV</option>
            <option value="4">Cobrable con recargo, con gastos administrativos y con 15% de ISV</option>
            <option value="5">Cobrable con recargo, con gastos administrativos y sin 15% de ISV</option>
            <option value="6">Cobrable con recargo, sin gastos administrativos y con 15% de ISV</option>
            <option value="7">Cobrable con recargo, sin gastos administrativos y sin 15% de ISV</option>
            ','','...'),' ',' '); 
        ?>
    </div> 
</div>
  ...
  <div class="table-responsive">  
    ...
    <div class="row">
          ...
          <div id="servicio_botones_agregar_guardar" class="botones_accion d-print-none bg-light px-3 py-2 mt-4 border-top ">
             ...
             <a href="#" onclick="procesar_servicio_oc('servicio_mant_ocobro.php?a=<?php echo $accion?>&g=1','formagr_oc','',<?php echo $tipo; ?>); return false;" class="btn btn-primary mr-2 mb-2 xfrm" ><i class="fa fa-check "></i> Crear Orden</a> 
...
actualizar_precios_venta(<?php echo $totlin ?>);

function actualizar_precios_venta(totlin) {
  var lineas = parseInt(totlin);
  var n=1;
  if (lineas>0) {
    lineas=lineas-1;
    while (n<=lineas) {
      $('#vdetli_'+n+' input[name="det_pc[]"]').trigger('change');
      n=n+1;
    }
  }
