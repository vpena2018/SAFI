...
if (app_enviar_email==true) {
    ...
    if (!isset($tipo_movimiento_texto)) {
        $tipo_movimiento_texto='Nueva Orden de Servicio';
    }
    ...
    <br>
     ...
     $email_enviar=$_SESSION['correo_orden_servicio_nueva'];
     if ($_SESSION['tienda_id']<>$correo_row["id_tienda"]) {
        $email_enviar=trim(get_dato_sql('tienda','correo_orden_servicio_nueva',' where id='.$correo_row["id_tienda"]));
     }
