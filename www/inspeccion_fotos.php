...
function render_foto_thumbnail($row, $mostrar_borrar = false,  $puede_borrar_por_permiso = false) {
    ...
    if ($es_imagen) {
        ...
        echo '<a href="#" onclick="' . $onclick . '"><img class="img img-thumbnail mb-3 mr-3" style="width: 180px; height: auto;" src="' . $src . '" data-cod="' . $id . '"></a> ';
        if ($mostrar_borrar) {
...
function thumb_agregar(archivo) {
    if (archivo != '' && archivo != undefined) {
        if (esImagen(archivo)) {
            $("#insp_fotos_thumbs").append('<a href="#" onclick="mostrar_foto(\'' + archivo + '\', \'local\'); return false;"><img class="img img-thumbnail mb-3 mr-3" src="uploa_d/thumbnail/' + archivo + '"></a> ');
        } else {
