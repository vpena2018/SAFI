...
if ($accion=="1") {
   // sleep(3);
    ...
    if (isset($_REQUEST['numero'])) { $tmpval=sanear_int($_REQUEST['numero']); if (!es_nulo($tmpval)){$filtros.=" and cotizacion.numero = ".GetSQLValue($tmpval,'int') ;}   }
    if (isset($_REQUEST['tipo'])) { $tmpval=sanear_int($_REQUEST['tipo']); if (!es_nulo($tmpval)){$filtros.=" and cotizacion.id_tipo_mant = ".GetSQLValue($tmpval,'int') ;}   }
    if (isset($_REQUEST['estado'])) { $tmpval=sanear_int($_REQUEST['estado']); if (!es_nulo($tmpval)){$filtros.=" and cotizacion.id_estado = ".GetSQLValue($tmpval,'int') ;}   }
    if (isset($_REQUEST['tienda'])) { $tmpval=sanear_int($_REQUEST['tienda']); if (!es_nulo($tmpval)){$filtros.=" and cotizacion.id_tienda = ".GetSQLValue($tmpval,'int') ;}   }
    ...
    $result = sql_select("SELECT cotizacion.id, cotizacion.fecha,  cotizacion.numero, cotizacion.numero_alterno
    ...
    where 1=1
    $filtros
    order by cotizacion.fecha desc, cotizacion.id desc
    "
    ...
    if ($result!=false){
        if ($result -> num_rows > 0) { 
            ...
            while ($row = $result -> fetch_assoc()) {
                $datos.='<tr>
                <td><a  href="#" onclick="cotizacion_abrir(\''.$row["id"].'\'); return false;" class="btn btn-sm btn-secondary btntxt">'.$row["numero"].'</a></td>
 ...
 <fieldset id="fs_forma">
    <div class="row">  
        ...
        <div class="col-sm">
         ...
         <div class="col-sm">
            <?php 
           echo campo("tipo","Tipo",'select',valores_combobox_db('cotizacion_tipo','','nombre','','','Todos'),' ',' onkeypress="buscarfiltro(event,\'btn-filtro\');"');
            ?>
         </div>
         <div class="col-sm">
            <?php 
           echo campo("estado","Estado",'select',valores_combobox_db('cotizacion_estado','','nombre','','','Todos'),' ',' onkeypress="buscarfiltro(event,\'btn-filtro\');"');
            ?>
    ...
    <div class="row"> 
            ...
            <div class="col-sm">
            <a id="btn-filtro" href="#" onclick="procesar_tabla_datatable('tablaver','tabla','cotizacion_ver.php?a=1','Cotizaciones','forma_ver'); return false;" class="btn btn-info mr-2 mb-2"><i class="fa fa-search"></i>  <?php echo "Buscar"; ?></a>
...
<script>
 ...
 //$('#pagina-botones').html('<a href="#" onclick="abrir_producto(\'0\'); return false;" class="btn btn-secondary mr-2 mb-2 xfrm" ><i class="fa fa-plus"></i> <?php echo 'Nuevo'; ?></a>');
    ...
    function cotizacion_abrir(codigo){
        
        get_page_switch('pagina','cotizacion_mant.php?a=v&cid='+codigo,'Cotización') ;
    }
