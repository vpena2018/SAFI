...
function agregar_servicio_detalle($vlin,$data_pv,$data_pc,$data_id,$data_alt,$data_desc,$det_id,$cantidad,$nota,$estado,$ocompra,$ocobro,$hora,$hora_atendido,$tipo_mant,$tipo) {
  $salidatxt="";
  $devueltoclass="";
  ...
  ?>
   <form id="forma_cambiarcmp" name="forma_cambiarcmp" class="needs-validation" novalidate>
    ...
    <?php 
      ...
      // Auditadob
      if ($nombre=='auditado'){    
         $salida.= campo("observaciones_adpc","Observaciones ADPC",'textarea','',' ',' rows="4" ');
      }
         ...
         WHERE id=$sid limit 1");
          ...
          if ($result!=false){
              if ($result -> num_rows > 0) { 
                 ...
                 $salida.= campo('id_tecnico1', $etiqueta,'select2',valores_combobox_db('usuario',$row["id_tecnico1"],'nombre',' where activo=1 and grupo_id=2 and tienda_id='.$_SESSION['tienda_id'],'','...'),' ',' required ',''); 
                 $salida.= campo('id_tecnico2', $etiqueta,'select2',valores_combobox_db('usuario',$row["id_tecnico2"],'nombre',' where activo=1 and grupo_id=2 and tienda_id='.$_SESSION['tienda_id'],'','...'),' ',' required ',''); 
                 $salida.= campo('id_tecnico3', $etiqueta,'select2',valores_combobox_db('usuario',$row["id_tecnico3"],'nombre',' where activo=1 and grupo_id=2 and tienda_id='.$_SESSION['tienda_id'],'','...'),' ',' required ',''); 
                 $salida.= campo('id_tecnico4', $etiqueta,'select2',valores_combobox_db('usuario',$row["id_tecnico4"],'nombre',' where activo=1 and grupo_id=2 and tienda_id='.$_SESSION['tienda_id'],'','...'),' ',' required ','');                      
              }
          }         
      } 
...
if ($elcodigo<>"") {
  ...
  if (isset($_REQUEST["id_estado"]) )  {
    ...
    if (intval($_REQUEST["id_estado"])>=3 and intval($_REQUEST["id_estado"])<>7) {
      ...
      //no dejar servicios repuestos pendientes antes de marcar completada
      if ($est_validacion==true) {                                                    //SUM( if(estado<>1 AND estado<>2 AND estado<>6 AND  estado<>8,0,1))
         $servicios_pendientes=intval(get_dato_sql("servicio_detalle","SUM(if(producto_tipo=3,if(estado<>9 AND estado<>6 AND estado<>7,1,0), if(estado<>3 AND estado<>4 AND estado<>5  AND estado<>7,1,0)))","WHERE id_servicio=".$elcodigo));// SUM(if(producto_tipo=3,if(estado=1,1,0), if(estado<=2 OR estado=6,1,0)))
        if ($servicios_pendientes>0) {
        
          $stud_arr[0]["pcode"] = 0;
          $stud_arr[0]["pmsg"] ="Hay $servicios_pendientes repuestos o servicios pendientes.<br> Debe marcar recibido, devolver o borrar repuestos pendientes";
          salida_json($stud_arr);
          exit;
        }
      }
  ...
  if ($_REQUEST["id_estado"]==21 or $_REQUEST["id_estado"]=="21") {//realizada
    if (isset($_REQUEST["observaciones"])){$sqlcampos.= " ,observaciones_realizado =".GetSQLValue(trim($_REQUEST["observaciones"]),"text");}
    ...
    <div class="card mb-3">
      ...
      <div class="card mb-3">
        <div class="card-header  bg-secondary text-white ">
            ...
            <a class="mr-3  btn btn-sm btn-info  d-print-none float-right" href="#" onclick="modalwindow2('Agregar Servicio','servicio_mant_repuesto.php?a=agr&tipo=3&cid='+$('#id').val()+'&estactual='+$('#id_estado').val()); return false;"><i class="fa fa-plus"></i> Agregar</a>
         </div>
        <div class="card-body"> 
      ...
      <div class="card mb-3">
        <div class="card-header  bg-secondary text-white ">
           ...
           <a class="mr-3 mb-1  btn btn-sm btn-info  d-print-none float-right" href="#" onclick="modalwindow2('Compra en Proceso','servicio_mant_repuesto.php?a=comppro&tipo=2&cid='+$('#id').val()); return false;"><i class="fa fa-truck"></i>Compras en Proceso</a>           
           ...
           <a class="mr-3 mb-1  btn btn-sm btn-info  d-print-none float-right" href="#" onclick="modalwindow2('Solicitar Repuesto','servicio_mant_repuesto.php?a=agr&tipo=2&cid='+$('#id').val()+'&estactual='+$('#id_estado').val()); return false;"><i class="fa fa-plus"></i> Solicitar</a>           
        </div>
        <div class="card-body">   
