  ...
  <form id="forma_cotiza_mod" name="forma_cotiza_mod" class="needs-validation" novalidate>
   ...
   <?php 
     $salida='';
       ...
       $result = sql_select("SELECT cotizacion_detalle.* ,entidad.nombre as prov 
       FROM cotizacion_detalle 
       LEFT OUTER JOIN entidad ON (cotizacion_detalle.id_proveedor=entidad.id) 
       where cotizacion_detalle.id_maestro=$cid   and cotizacion_detalle.id=$did
       limit 1");
   ...
   <div class="row">
         ...
         <div class="botones_accion d-print-none bg-light px-3 py-2 mt-4 border-top ">
           ...
           <a href="#" onclick="procesar_forma_cotiza_mod('cotizacion_mant_detalle.php?a=mod_g&cid=<?php echo $cid;?>&did=<?php echo $did;?>','forma_cotiza_mod',''); return false;" class="btn btn-primary mr-2 mb-2 xfrm" ><i class="fa fa-check "></i> Guardar</a>
...
if ($accion =="mod_g") {
 ...
 { $sqlcampos.= " , producto_tipo =".GetSQLValue($producto['tipo'],"int"); } 
  ...
  $sql="update cotizacion_detalle set ".$sqlcampos." where id=".$did." and cotizacion_detalle.id_maestro=$cid  limit 1";
