...
function  cargar_detalle_oc($cid,$tipo,$sql_detalle){
    ...
    $averias_result = sql_select("SELECT * FROM averia_detalle where  ($sql_detalle) and id_maestro=$cid  and  estado<>4 order by producto_tipo desc,id ");
        ...
        if ($averias_result->num_rows > 0) { 
          while ($detalle = $averias_result -> fetch_assoc()) {
            ...
            echo agregar_averia_detalle_oc($lin,$detalle["precio_venta"],$detalle["precio_costo"],$detalle["id_producto"],$detalle["producto_codigoalterno"],$detalle["producto_nombre"],$detalle["id"],$detalle["cantidad"],$detalle["producto_nota"],$detalle["estado"]);
  ...
  function agregar_averia_detalle_oc($vlin,$data_pv,$data_pc,$data_id,$data_alt,$data_desc,$det_id,$cantidad,$nota,$estado) {
    $salidatxt="";
    $devueltoclass="";
...
if ($accion =="ocompra") { //crear ORDEN COMPRA
           ...
           LIMIT 1");
            ...
            //Calculos
             ...
             $tmpsql="";$tmpsql2="";
               ...
               $tmpsql="UPDATE averia_detalle SET id_oc=:codigo $tmpsqlcosto WHERE id=".$detalleid." LIMIT 1";
   ...
   if ($verror=="") {
    ...
    if ($result!=false){
        ...
        VALUES ( $cid,  ".$_SESSION['usuario_id'].", 'Crear Orden de Compra', NOW(), 'Averia #$numero_oc')");
  ...
  <div class="table-responsive">  
    ...
    <div class="row">
          ...
          <div id="averia_botones_agregar_guardar" class="botones_accion d-print-none bg-light px-3 py-2 mt-4 border-top ">
             ...
             <a href="#" onclick="procesar_averia_oc('averia_mant_oc.php?a=<?php echo $accion?>&g=1','formagr_oc','',<?php echo $tipo; ?>); return false;" class="btn btn-primary mr-2 mb-2 xfrm" ><i class="fa fa-check "></i> Crear Orden</a> 
